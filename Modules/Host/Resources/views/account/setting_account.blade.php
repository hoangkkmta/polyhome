...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('host.submit.setting.account') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="facebook" value="{{ old('facebook', $account->facebook) }}">
                            @error('facebook')
                            <span class="mt-3 errorMsg text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            <label>Địa chỉ zalo:</label>
                            <input type="text" class="form-control" name="zalo" value="{{ old('zalo', $account->zalo) }}">
                            @error('zalo')
                            <span class="mt-3 errorMsg text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group d-flex justify-content-center">
                            <a href="{{ route('host.dashboard') }}" class="btn btn-lg btn-default mr-3">Trở lại</a>
